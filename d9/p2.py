...
for i in range(input_length):
    ...
    if current_element in sums:
        ...
        # if not, set target to current element and break
...
for i in range(input_length):
    # Get all contiguous sums starting from i
    current_sums = list(itertools.accumulate(input_list[i:]))
    # If our target is in that list
    if target in current_sums:
        # Get the index of that sum; this is the offset from
        # initial element
        x = current_sums.index(target)
        # Make a sublist from i to x
        sublist = input_list[i:i+x+1]
        # Sort sublist
        sublist.sort()
        # Output sum of first and last elements of sorted list
        print("Found ", sublist[0] + sublist[-1])
        break
